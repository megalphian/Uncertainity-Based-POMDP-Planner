...
class KalmanEstimator:
    ...
    def get_estimates(self, no_iters, init_state, inputs, measurements):
        cov = np.identity(2)
        ...
        for i in range(no_iters):
            ...
            noise_term = (C @ sigma_cT) + self.N[i]

            K = (A @ sigma_cT) * np.linalg.inv(noise_term)
            update = (A - (K @ C))

            measurement_i = np.asarray([[measurements[i]]])
            input_i = np.asarray([[inputs[i][0]], [inputs[i][1]]])

            x = (update @ x) + (K @ measurement_i) + input_i

            # Add process noise later
            cov = (update @ (cov @ np.linalg.inv(update))) + (K @ self.N[i] @ np.transpose(K))
